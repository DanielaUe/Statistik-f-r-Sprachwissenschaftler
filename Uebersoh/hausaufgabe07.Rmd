...
Welche Verteilung sieht am fairsten aus? Warum? Rot (also mit einer Standardabweichung von drei), weil die Wahrscheinlichkeit für Durchfallen niedrig ist (niedriger als bei den anderen Standardabweichungen), dafür aber natürlich auch die Wahrscheinlichkeit für eine gute Punktzahl nicht so hoch ist. Bei den anderen Standardabweichungen sind die Kurven flacher, aber auch die Wahrscheinlichkeiten für die jeweiligen Endpunkte höher, also nicht besonders fair, wenn viele durchfallen.
...
pnorm(13, mean=mu, sd=3, lower.tail=FALSE)
for(s in c(3,4,5) ){
  bestehen.ausgezeichnet <- pnorm(13,mean=mu,sd=s,lower.tail=FALSE)
  output <- paste("Bei einer Standabweichung von",s, "bestehen",bestehen.ausgezeichnet*100,"% ausgezeichnet.")
  print(output)
}


Wie steht die Anzahl guter Noten in Beziehung zur Anzahl schlechter Noten? 
Es gibt eine viel schlechtere Wahrscheinlichkeit für die guten Noten (5, 11 und 16 %) als für die schlechten

Warum?
Die schlechten Noten gehen von 0-5 Punkten, die im Gegensatz zu 13-15 Punkten stehen. Die schlechten Noten haben also mehr Werte als die guten und deshalb auch eine größere Wahrscheinlichkeit des Auftretens.
...
Bei der Verteilung der Notenpunkte (0-15) liegt der Gipfel für gute Noten rechts und es ist deshalb linksschief.
...
(pnorm(9,mean=mu,sd=3) - pnorm(7,mean=mu,sd=3) ) * n
13 Studenten bekommen zwischen 7 und 9 Notenpunkten. 
...
2. Wie viele Studenten bekommen zumindest 10 NP?
  ...
  (pnorm(16,mean=mu,sd=3) - pnorm(10, mean=mu, sd=3)) * n 
    Ca. 12 Studenten bekommen zumindest 10 Notenpunkte.

3. Wie viele Studenten bekommen weniger als 10 NP?

  (1 - (pnorm(16,mean=mu,sd=3) - pnorm(10, mean=mu, sd=3))) * n
    Ca. 38 Studenten bekommen weniger als 10 Notenpunkte.

4. Wie viele Studenten bekommen weniger als 8 NP?

  pnorm(8,mean=mu,sd=3) * n
...
qnorm(0.5,mean=mu,sd=3)
mehr als 8 Notenpunkte
...
qnorm(0.99,mean=mu,sd=3) 
14,98 Notenpunkte bekommen.
...
Der kritische Wert für einen einseitigen $z$-Test ist 1,554774.
qnorm(0.94)

Die kritischen Werte für einen zweiseitigen $z$-Test sind $\pm$ 1,880794.
qnorm(0.97)

### Gibt es einen Unterschied?
Bei diesem Dozenten ist die Verwaltung wirklich unsicher, ob und was für einen Unterschied es geben könnte. (Welche Testart sollte man hier nutzen?) Zweiseitigen z-Test, um in beide Richtungen gucken zu können
...
H0 wird abgelehnt, wenn Betrag von z größer gleich dem kritischen Wert von 1,880794.
n<-7 
m<-10
mu<-8
sd<-3
...
Da z=1,76< 1,88 gibt es keinen signifikanten Unterschied (also H0 beibehalten).
...
Da z=4,71 > 1,88 gibt es einen signifikanten Unterschied (also H0 ablehnen).
...
Die Verwaltung ist auch auf einen anderen Dozenten aufmerksam geworden, weil manche Studenten behaupten, er würde zu streng benoten. (Welche Testart sollte man hier nutzen?) Einseitiger Test
...
H0 wird abgelehnt, wenn Betrag von z größer gleich dem kritischen Wert von 1,554774.
n<-20
m<-7
mu<-8
sd<-3
...
Es gibt keinen signifikanten Unterschied, da Betrag von -1,49=1,49 < als 1,55 (also H0 annehmen).
...
Es gibt einen signifikanten Unterschied, da der Betrag von -1,67=1,67 > als 1,55 (also H1 annehmen).
